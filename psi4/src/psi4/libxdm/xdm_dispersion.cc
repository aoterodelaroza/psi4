...
#include "psi4/libdisp/dispersion_defines.h"
...
std::shared_ptr<XDMDispersion> XDMDispersion::build(const std::string &name, double s6, double alpha6, double sr6) {
    ...
    if (to_upper_copy(name) == "D1") {
        ...
        throw PSIEXCEPTION("XDMDispersion: Unknown -D type specified");
    }
}
...
std::string XDMDispersion::print_energy(std::shared_ptr<Molecule> m) {
    ...
    s << "   " << name_ << " XDMDispersion Energy: " << e << " [Eh]" << std::endl;
...
std::string XDMDispersion::print_gradient(std::shared_ptr<Molecule> m) {
    ...
    s << "   " << name_ << " XDMDispersion Gradient ([a.u.]): " << std::endl << std::endl;
...
std::string XDMDispersion::print_hessian(std::shared_ptr<Molecule> m) {
    ...
    s << "   " << name_ << " XDMDispersion Hessian ([a.u.]): " << std::endl << std::endl;
    for (int k = 1; k <= m->natom(); k++) {
...
double XDMDispersion::compute_energy(std::shared_ptr<Molecule> m) {
    ...
    printf("xxxx hello from inside libxdm!! xxxx\n");
...
std::shared_ptr<Vector> XDMDispersion::set_atom_list(std::shared_ptr<Molecule> mol) {
